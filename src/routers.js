...
router
  ...
  .get("/logs/:id", check, async (req, res) => {
    ...
    res.render(res.locals.mobile ? "mobile/logs" : "desktop/logs", {
      logs,
  ...
  .get("/stats/", check, async (req, res) => {
    ...
    res.render(res.locals.mobile ? "mobile/stats" : "desktop/stats", {
      stats,
      username: user.username,
      user,
    });
  ...
  .get("/start/:id", check, async (req, res) => {
    ...
    if (dockers.has(name)) {
      ...
      if (data.id !== "") {
        ...
        await docker
          .startDocker({ username: user.username, name: name })
          .then(async (id) => {
            data.running = true;
            data.id = id;
            dockers.set(name, data);
    ...
    if ("verify" in req.body) {
      docker
        .createDocker(
          code,
          name,
          user.dockers[code].version,
          user.dockers[code].temp
        )
        .then(async () => {
          const dockers = ObjToMap(user.dockers);
          ...
          user.messages.push({
            ...
            author: "Server",
            message:
              "Your code has been verified and a docker image has been created",
          });
...
router
  ...
  .all(check)
  .get((req, res) => {
    res.render(res.locals.mobile ? "mobile/addDocker" : "desktop/addDocker", {
      message: req.flash("add-docker"),
      user: req.session.user,
    });
  })
  .post(async (req, res) => {
    ...
    if (files == null || files == undefined) return res.redirect("/dashboard");
    ...
    if (
      !fs.existsSync(path.join(__dirname, "verify", req.session.user.username))
    )
      await fs.mkdirSync(
        path.join(__dirname, "verify", req.session.user.username),
        { recursive: true }
      );
    await files.code.mv(
      path.join(
      ...
      async (err) => {
        ...
        var dockers;
        if (Object.entries(user.dockers).length > 0)
          dockers = ObjToMap(user.dockers);
        ...
        console.log({ dockers: dockers.get(body.name) });

        user.dockers = MapToObj(dockers);

        console.log({ user: user.dockers });
...
router
  .route("/login")
  .get((req, res) => {
    return res.render("login", { message: req.flash("login-message") });
  })
  .post(async (req, res) => {
